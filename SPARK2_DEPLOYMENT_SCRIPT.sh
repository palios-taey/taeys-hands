...
if curl -s http://192.168.x.11:8080/v1/meta > /dev/null 2>&1; then
    success "Weaviate is running on 192.168.x.11:8080"
else
...
EOF
    ...
    for i in {1..60}; do
        if curl -s http://192.168.x.11:8001/v1/models > /dev/null 2>&1; then
...
MODELS=$(curl -s http://192.168.x.11:8001/v1/models | python3 -c "import sys, json; print(json.load(sys.stdin)['data'][0]['id'])" 2>/dev/null || echo "")
if [[ -z "$MODELS" ]]; then
...
class ParallelEmbeddingPipeline:
    ...
    def __init__(
        self,
        embedding_base_url: str = "http://192.168.x.11:8001/v1",
        embedding_model: str = "Qwen/Qwen3-Embedding-8B",
        weaviate_host: str = "192.168.x.11",
