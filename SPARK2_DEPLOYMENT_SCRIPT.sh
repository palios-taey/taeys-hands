...
# Target: Spark #2 (10.x.x.80 / REDACTED_LAN_IP)
...
if curl -s http://REDACTED_LAN_IP:8080/v1/meta > /dev/null 2>&1; then
    success "Weaviate is running on REDACTED_LAN_IP:8080"
else
...
CUDA_VISIBLE_DEVICES=0,1 vllm serve \
    ...
    --enforce-eager \
    >> <OPERATOR_HOME>/logs/vllm-embedding.log 2>&1 &
...
EOF
    ...
    for i in {1..60}; do
        if curl -s http://REDACTED_LAN_IP:8001/v1/models > /dev/null 2>&1; then
...
MODELS=$(curl -s http://REDACTED_LAN_IP:8001/v1/models | python3 -c "import sys, json; print(json.load(sys.stdin)['data'][0]['id'])" 2>/dev/null || echo "")
if [[ -z "$MODELS" ]]; then
...
BUILDER_DIR="<OPERATOR_HOME>/builder-taey"
...
class ParallelEmbeddingPipeline:
    ...
    def __init__(
        self,
        embedding_base_url: str = "http://REDACTED_LAN_IP:8001/v1",
        embedding_model: str = "Qwen/Qwen3-Embedding-8B",
        weaviate_host: str = "REDACTED_LAN_IP",
...
Restart=always
RestartSec=10
StandardOutput=append:<OPERATOR_HOME>/logs/vllm-embedding.log
StandardError=append:<OPERATOR_HOME>/logs/vllm-embedding.log
...
async def test():
    client = AsyncOpenAI(
        base_url="http://REDACTED_LAN_IP:8001/v1",
        api_key="EMPTY"
    )
...
echo "  tail -f <OPERATOR_HOME>/logs/vllm-embedding.log"
echo
