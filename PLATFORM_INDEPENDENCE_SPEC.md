...
v5, 2026-07-03, taeys-hands (Fable). Jesse-directed architecture: "everyone needs their own YAML, their own driver and their own monitor... Each platform is completely independent."
Status: v1 BLOCKED (8) → v2 resolved-8/BLOCKED-3 → v3 resolved-3/BLOCKED on the CLASS (piecemeal curation: each round exposed the next uncovered fix — 9761aee0, f991d951, 2da240dd) → v4 generative machinery (class closed per council: 2 PASS/3 warn) BLOCKED on 3 precision defects → v5: unreachability requires GitNexus execution-flow proof (grep banned), full-history disposition window (no epoch), YAML-discipline ledger item (de248ee8/d4b09dc4) → re-review → ratification. Council logs archived in task evidence. Gates all `consult-platform-independence` work.
...
- **GENERATIVE DISPOSITION TABLE (v4 — supersedes curated scoping; council-required root-cause shape):** parity is NOT proven against a hand-curated ledger or a named module subset — every council round against curated lists found the next uncovered fix (9761aee0, f991d951, 2da240dd). Instead, mechanically: for EVERY module code is moved out of or retired from (`drivers/base.py`, `completion.py`, AND every section-4 conditionally-leaf module touched by the audit-push or split — `runtime.py`, `snapshot.py`, `planner.py`, `interact.py`, `primitives.py`, `platforms_runtime.py`, `input.py`, `atspi.py`, plus the `cli.py`/`orchestrator.py` push-downs), run `git log --oneline -- <module>` (FULL history — no epoch; council v4: a date cutoff structurally excludes older design-discipline fixes) at the pinned source SHA and give EVERY commit a disposition row: **inlined** (file:line citation in the package), **retained-in-orchestrator** (named in section 7), **data-moved-to-registry**, or **unreached-with-evidence** — where unreachability evidence MUST be a GitNexus execution-flow proof (`gitnexus impact`/`gitnexus context` on the symbol, per the canonical 6SIGMA workflow) or an equivalent AST call-graph artifact; **raw grep is explicitly INSUFFICIENT and rejected at the r5 gate** (Python dynamic dispatch/getattr/subclass method-resolution make text search unsound as an unreachability oracle — council v4, 4 roles). The table + the slice-diff (inclusion parity) + the omission manifest (every un-inlined method of the moved surface) ship in the PR and are reviewed at the r5 gate. A commit with no row fails the gate. Nothing is dropped by unaudited judgment, and there is no curated subset left to be incomplete.
...
- **THE FIX LEDGER (council-required; the checklist every package must clear):** each guard below is either evidenced PRESENT in the package (file:line citation against the pinned source) or recorded ABSENT-BY-DESIGN with rationale in the PR. A happy-path consult alone NEVER closes a package task.
  ...
  - `de248ee8` / `d4b09dc4` **detection-in-YAML-not-code discipline** (mode-active detection moved OUT of base.py Python into YAML specs): every package's driver.py/monitor.py must be checked at the r5 gate for resurrected hardcoded AT-SPI selectors/mode-detection in code — detection stays in the package's YAML (invariant 1)
