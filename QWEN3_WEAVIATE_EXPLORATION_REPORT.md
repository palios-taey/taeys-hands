...
**Weaviate Endpoint**: `http://192.168.x.11:8080` (Spark #2 internal IP)
...
- Expected URL: `http://192.168.x.11:8001/v1`
...
```json
{
  "mcpServers": {
    "unified-memory": {
      "env": {
        "WEAVIATE_URL": "http://192.168.x.11:8080",
...
**Spark #2 (192.168.x.11 - EMBEDDING NODE)**:
- Weaviate: http://192.168.x.11:8080
- vLLM (Qwen3-Embedding-8B): http://192.168.x.11:8001
...
3. **Spark 1 Connectivity**:
   ...
   - Or incorrect IP (should it be 192.168.x.10?)
...
- Edit `unified_query_canonical.py:75` → `http://192.168.x.11:8000/v1`
...
├─ 192.168.x.10  Spark #1 HEAD (Ray cluster, primary orchestration)
└─ 192.168.x.11  Spark #2 WORKER (embeddings, Weaviate)

Services:
Spark #1 (192.168.x.10):
  - Neo4j: bolt://localhost:7687
  - Elasticsearch: http://localhost:9200
  - Qwen3-Coder-30B: http://localhost:8000

Spark #2 (192.168.x.11):
  - Weaviate: http://192.168.x.11:8080 ✅
  - Qwen3-Embedding-8B: http://192.168.x.11:8001 (should be, currently 8000 ⚠️)
```
...
- May need to use internal IP 192.168.x.10
