...
**Repository**: `/Users/REDACTED/builder-taey`
...
**Target Architecture**:
- **Spark #1 (10.x.x.68)**: Neo4j + Elasticsearch + vLLM (Qwen3-Coder-30B orchestration)
- **Spark #2 (10.x.x.80)**: Weaviate + vLLM (Qwen3-Embedding-8B)
...
Environment="PERSISTENCE_DATA_PATH=<OPERATOR_HOME>/.weaviate/data"
```
...
**Weaviate Endpoint**: `http://REDACTED_LAN_IP:8080` (Spark #2 internal IP)
...
- Expected URL: `http://REDACTED_LAN_IP:8001/v1`
...
```json
{
  "mcpServers": {
    "unified-memory": {
      "env": {
        "WEAVIATE_URL": "http://REDACTED_LAN_IP:8080",
...
**Spark #1 (localhost, 10.x.x.80 - PRIMARY NODE)**:
- Neo4j: bolt://localhost:7687
- Elasticsearch: http://localhost:9200
- vLLM (Qwen3-Coder-30B): http://localhost:8000

**Spark #2 (REDACTED_LAN_IP - EMBEDDING NODE)**:
- Weaviate: http://REDACTED_LAN_IP:8080
- vLLM (Qwen3-Embedding-8B): http://REDACTED_LAN_IP:8001
...
### On Spark 2 (10.x.x.80) - VERIFIED VIA SSH
...
- builder-taey repository NOT present at `<OPERATOR_HOME>/builder-taey/`
...
3. **Spark 1 Connectivity**:
   ...
   - Or incorrect IP (should it be REDACTED_LAN_IP?)
...
- Edit `unified_query_canonical.py:75` → `http://REDACTED_LAN_IP:8000/v1`
...
curl http://REDACTED_LAN_IP:8001/v1/models
```
...
```
External IPs (REDACTED_LAN_IP):
├─ 10.x.x.68  Spark #1 (EXTERNAL - SSH TIMEOUT)
└─ 10.x.x.80  Spark #2 (EXTERNAL - ACCESSIBLE)

Internal IPs (REDACTED_LAN_IP):
├─ REDACTED_LAN_IP  Spark #1 HEAD (Ray cluster, primary orchestration)
└─ REDACTED_LAN_IP  Spark #2 WORKER (embeddings, Weaviate)

Services:
Spark #1 (REDACTED_LAN_IP):
  - Neo4j: bolt://localhost:7687
  - Elasticsearch: http://localhost:9200
  - Qwen3-Coder-30B: http://localhost:8000

Spark #2 (REDACTED_LAN_IP):
  - Weaviate: http://REDACTED_LAN_IP:8080 ✅
  - Qwen3-Embedding-8B: http://REDACTED_LAN_IP:8001 (should be, currently 8000 ⚠️)
```
...
- External IP 10.x.x.68 timing out
- May need to use internal IP REDACTED_LAN_IP
...
2. **Spark 1 Connectivity**: How to troubleshoot SSH timeout to 10.x.x.68?
...
7. **Network Configuration**: Should we use internal IPs (REDACTED_LAN_IP) for everything?
