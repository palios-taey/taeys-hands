"""Gemini platform package driver.

This file inlines Gemini's current effective driver behavior plus the lifecycle
methods it reaches, so Gemini owns its driver and monitor code in this package.
"""
...
from consultation_v2.platforms.gemini.monitor import COMPLETE, DEEP_MODES, GeminiCompletionDetector
...
class _GeminiInlineBase:
    ...
    @staticmethod
    def _reset_detector_after_intermediate(detector: GeminiCompletionDetector) -> None:
...
class GeminiConsultationDriver(_GeminiInlineBase):
    platform = 'gemini'

    # run() is the shared two-phase template on _GeminiInlineBase (FLOW §10):
