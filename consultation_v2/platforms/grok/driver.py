"""Grok platform package driver.

This file inlines Grok's current effective driver behavior plus the lifecycle
methods it reaches, so Grok owns its driver and monitor code in this package.
"""
...
from consultation_v2.platforms.grok.monitor import COMPLETE, DEEP_MODES, GrokCompletionDetector
...
class _GrokInlineBase:
    ...
    @staticmethod
    def _reset_detector_after_intermediate(detector: GrokCompletionDetector) -> None:
    ...
    def monitor_generation(
        ...
        """Poll until the response completes, using the Grok stop-transition
        detector (consultation_v2.platforms.grok.monitor.GrokCompletionDetector) - the single
        ...
        detector = GrokCompletionDetector(mode=detector_mode)
    ...
    def run(self, request: ConsultationRequest) -> ConsultationResult:
        ...
        with pause_display_watchdog(self.platform, self._display()):
            ...
            if result.ok and result.response_text and self.reject_prompt_echo_response(
                ...
                source='grok_package_run_delivery_gate',
            ):
                result.ok = False
            return result
...
class GrokConsultationDriver(_GrokInlineBase):
    platform = 'grok'

    # ------------------------------------------------------------------
    # Orchestration
    # ------------------------------------------------------------------
    # run() is the inlined two-phase template from the pinned source tree:
    ...
    def setup_and_send(
        ...
        """LOCKED phase (FLOW Section 10): navigate -> mode -> attach -> prompt ->
    ...
    def monitor_and_extract(
        ...
        """UNLOCKED phase (FLOW Section 10): wait for completion -> extract -> store.
    ...
    # ------------------------------------------------------------------
    # Step 6 - wait for completion - Grok package stop-transition detector.
