...
def _attachment_rule() -> tuple[str, str]:
    ...
    if attachment.get('open_method') != 'focus_and_key_open':
        raise ValueError('grok attachment open_method must be focus_and_key_open')
    open_key = attachment.get('open_key')
    if not isinstance(open_key, str) or not open_key:
        raise ValueError('grok focus_and_key_open requires an exact open_key')
...
def key_requires_state(key: str) -> bool:
    ...
    if key.casefold() == open_key.casefold() and key != open_key:
        raise ValueError(
            f'grok attachment opening key must exactly equal YAML {open_key!r}'
        )
    return key == open_key


def validate_key_state(key: str, snapshot: Snapshot) -> None:
    if snapshot.platform != 'grok':
        raise ValueError(
            f'Grok key validation received platform {snapshot.platform!r}'
        )
    trigger_key, open_key = _attachment_rule()
    if key != open_key:
        raise ValueError(
    ...
    if len(matches) != 1:
        raise ValueError(
            f'grok {trigger_key} matched {len(matches)} elements; expected one'
        )
    ...
    if declared is None or expected not in declared['allowed_now']:
        raise ValueError(
            f'grok YAML opening key {key!r} is not allowed in the fresh '
