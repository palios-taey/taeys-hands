...
| Consultation inputs | [`PACKET_CONTRACT.md`](PACKET_CONTRACT.md) | Exactly one governance bundle, one task bundle, and one brief on-screen prompt. | Canonical contract. `identity.consolidate_attachments` now emits Bundle A + Bundle B (no one-package fallback); mechanical gate: `python3 -m consultation_v2.validators.lint_packet_contract`. Full production conformance still requires a live two-chip send receipt. |
