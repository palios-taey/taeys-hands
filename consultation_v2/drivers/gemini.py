...
class GeminiConsultationDriver(BaseConsultationDriver):
    ...
    def run(self, request: ConsultationRequest) -> ConsultationResult:
        ...
        if target_url:
            ...
            result.add_step('navigate', navigated, 'Navigated to Gemini session target', target_url=target_url, snapshot=snap.serializable())
            if not navigated:
                return result
    ...
    def select_model_mode_tools(self, request: ConsultationRequest, result: ConsultationResult) -> bool:
        ...
        if requested_model and requested_model in workflow.get('model_targets', {}):
            ...
            if not picker:
                result.add_step('select_model', False, 'Gemini mode picker not found', snapshot=snap.serializable())
                return False
        ...
        if requested_mode and requested_mode in workflow.get('tool_targets', {}):
            ...
            result.add_step('select_mode', True, 'Gemini mode left unchanged/default', requested_mode=request.mode)

        for tool_name in request.tools:
            normalized = tool_name.strip().lower().replace(' ', '_')
            target_key = workflow.get('tool_targets', {}).get(normalized)
            if not target_key:
                result.add_step('select_tool', False, f'Gemini tool {tool_name!r} not mapped in Consultation V2 YAML')
                return False
            snap = self.runtime.snapshot()
            tools_button = self.find_first(snap, 'tools_button')
    ...
    def attach_files(self, request: ConsultationRequest, result: ConsultationResult) -> bool:
        for file_path in request.attachments:
            ...
            if not trigger:
                result.add_step('attach', False, f'Gemini upload menu trigger missing for {abs_path}', snapshot=snap.serializable())
                return False
            ...
            result.add_step('attach', verified, f'Gemini attached {os.path.basename(abs_path)}', file=abs_path, snapshot=verify_snap.serializable())
            if not verified:
                return False
    ...
    def enter_prompt(self, request: ConsultationRequest, result: ConsultationResult) -> bool:
        ...
        if not self.runtime.click(input_el, strategy='atspi_first'):
            result.add_step('prompt', False, 'Gemini input focus click failed', snapshot=snap.serializable())
            return False
        time.sleep(0.3)
        ...
        result.add_step('prompt', verified, 'Gemini prompt entered', snapshot=verify_snap.serializable())
        return verified
    ...
    def send_prompt(self, request: ConsultationRequest, result: ConsultationResult) -> bool:
        ...
        if not send_button:
            result.add_step('send', False, 'Gemini send button not found', snapshot=snap.serializable())
            return False
        clicked = self.runtime.click(send_button, strategy='atspi_first')
        stop_seen = self.runtime.wait_until(lambda: self.runtime.snapshot().has('stop_button') or self.runtime.snapshot().has('start_research'), timeout=30, interval=0.6)
        ...
        if start_button:
            ...
            time.sleep(1.0)
            stop_seen = self.runtime.wait_until(lambda: self.runtime.snapshot().has('stop_button'), timeout=30, interval=0.8)
        ...
        result.add_step('send', verified, 'Gemini send validated by stop/start-research flow and URL capture', url_before=before, url_after=result.session_url_after, start_research_clicked=post_send_clicked, snapshot=verify_snap.serializable())
        return verified
    ...
    def monitor_generation(self, request: ConsultationRequest, result: ConsultationResult) -> bool:
        ...
        result.add_step('monitor', verified, 'Gemini response completed', stop_seen=seen_stop, snapshot=verify_snap.serializable())
        return verified
    ...
    def extract_primary(self, request: ConsultationRequest, result: ConsultationResult) -> bool:
        ...
        if not copy_button:
            result.add_step('extract_primary', False, 'Gemini copy button not found', snapshot=snap.serializable())
            return False
        if not self.runtime.click(copy_button, strategy='atspi_first'):
            result.add_step('extract_primary', False, 'Gemini copy button click failed', snapshot=snap.serializable())
            return False
        time.sleep(0.4)
        content = self.runtime.read_clipboard().strip()
        result.response_text = content
        verified = bool(content)
        result.add_step('extract_primary', verified, 'Gemini response copied to clipboard', characters=len(content), preview=content[:200])
        return verified
    ...
    def extract_additional(self, request: ConsultationRequest, result: ConsultationResult) -> bool:
        ...
        if not share_export:
            result.add_step('extract_additional', True, 'Gemini no additional export surface was visible')
            return True
        ...
        content = self.runtime.read_clipboard().strip()
        if content:
            result.extractions.append(ExtractedArtifact(name='gemini_export.md', content=content, kind='report_export', metadata={'source': 'share_export_copy_content'}))
            result.add_step('extract_additional', True, 'Gemini additional export copied', characters=len(content), preview=content[:200])
            return True
        result.add_step('extract_additional', False, 'Gemini additional export clipboard was empty')
        return False
    ...
    def store_in_neo4j(self, request: ConsultationRequest, result: ConsultationResult) -> bool:
        if request.no_neo4j or neo4j_client is None:
            ...
            result.add_step('store', True, 'Gemini Neo4j storage skipped', storage=result.storage)
            return True
        try:
            ...
            result.add_step('store', True, 'Gemini response stored in Neo4j', storage=result.storage)
            return True
