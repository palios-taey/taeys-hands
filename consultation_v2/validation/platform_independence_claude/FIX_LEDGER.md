...
| Starved AT-SPI read is not completion | ⚠️ CORRECTED (cannot-lie, r5): the package-local base monitor has the `MONITOR_MIN_HEALTHY_RAW_COUNT` guard, BUT Claude's ACTIVE completion path is the subclass monitor override, which has NO starved-read guard — on BOTH main AND the package. This is a PARITY-PRESERVED ABSENT gap (the guard was never present on Claude's active path on main either), not a preserved protection. Behavior-parity holds (absence matches main); the `MONITOR_MIN_HEALTHY_RAW_COUNT = 25` constant is now defined at module top so the base-monitor reference resolves and the guard is one refactor away from being wired into the subclass path. | `consultation_v2/platforms/claude/driver.py:2655`, `consultation_v2/platforms/claude/driver.py:2660`, `consultation_v2/platforms/claude/driver.py:2663` |
...
| Claude artifact extraction uses mapped controls, not blind coordinates | ⚠️ CORRECTED (cannot-lie, per val-claude DEFINITIVE @c771205a): artifact extraction is NOT confirmed handled — it FAILED at runtime. The package's mapped `artifact_copy_keys` + raw AT-SPI candidates yielded no artifact content (`extract_additional` red: "no configured control yielded artifact content"), while `extract_primary`/prose passed. The blind focus/ratio helper from main is deliberately ABSENT (removed per w2d "remove blind artifact coordinates"). GATEKEEPER BASE-DIFF MUST ARBITRATE regression-vs-pre-existing: if the shared-base/main claude extracted artifacts via that removed blind helper → this is a REGRESSION (fix before merge, like the NameError + import bugs); if main also could not extract artifacts → pre-existing gap → merge + follow-up. Follow-up filed: map the live right-panel artifact Copy control (same class as the perplexity `Copy contents`→`Copy` UI-drift). | `consultation_v2/platforms/claude/driver.py:4481`, `consultation_v2/platforms/claude/driver.py:4555`, `consultation_v2/platforms/claude/claude.yaml:631` |
