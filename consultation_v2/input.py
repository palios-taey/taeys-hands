"""Keyboard and mapped pointer input via xdotool, clipboard paste via xsel+Ctrl+V."""
...
def click_at(x: int, y: int, timeout: int = 5) -> bool:
    """Move to one mapped screen point and click the primary pointer button once."""
    try:
        r = subprocess.run(
        ...
        if r.returncode != 0:
            logger.warning(f"Mapped click at ({x},{y}) failed: {r.stderr.decode()}")
        return r.returncode == 0
    except subprocess.TimeoutExpired:
        logger.error(f"Mapped click at ({x},{y}) timed out")
        return False
    except Exception as e:
        logger.error(f"Mapped click at ({x},{y}) error: {e}")
        return False
