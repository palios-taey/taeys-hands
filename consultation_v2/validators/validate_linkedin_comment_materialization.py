...
SCHEMA = REPO_ROOT / 'consultation_v2/platforms/linkedin/comment-approval-source.schema.json'
...
def expected_artifacts(private_root: Path, value: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    normalized_hash = hashlib.sha256(' '.join(value['text'].strip().split()).encode()).hexdigest()
...
def validate_happy_path(case_root: Path) -> None:
    root, source, value = fixture(
        case_root,
        raw_bytes=(json.dumps(approval(), indent=2, ensure_ascii=False) + '\n').encode(),
    )
    ...
    assert hashlib.sha256(actual_receipt_bytes).hexdigest() == result['gate_receipt_sha256']
    assert hashlib.sha256(actual_transaction_bytes).hexdigest() == result['transaction_sha256']
...
def validate_source_refusals(base: Path) -> int:
    ...
    cases: list[tuple[str, dict[str, Any] | None, bytes | None, int]] = [
        ...
        ('bad-body-hash', {**valid, 'selected_post_body_sha256': 'B' * 64}, None, 0o400),
        ...
        ('control-author', {**valid, 'expected_author_name': 'Private\nAuthor'}, None, 0o400),
        ('empty-text', {**valid, 'text': ''}, None, 0o400),
        ('nul-text', {**valid, 'text': 'bad\x00text'}, None, 0o400),
        ('no-gates', {**valid, 'gates': []}, None, 0o400),
        ('failed-gate', {**valid, 'gates': [{'gate': 'cannot_lie', 'passed': False, 'ev': {}}]}, None, 0o400),
        ('bad-gate-name', {**valid, 'gates': [{'gate': 'Cannot Lie', 'passed': True, 'ev': {}}]}, None, 0o400),
        ('bad-gate-evidence', {**valid, 'gates': [{'gate': 'cannot_lie', 'passed': True, 'ev': 'yes'}]}, None, 0o400),
        ('extra-gate-field', {**valid, 'gates': [{'gate': 'cannot_lie', 'passed': True, 'ev': {}, 'extra': True}]}, None, 0o400),
        ('duplicate-gate', {**valid, 'gates': [valid['gates'][0], valid['gates'][0]]}, None, 0o400),
    ...
    for field in tuple(valid):
        cases.append((f'missing-{field}', {key: item for key, item in valid.items() if key != field}, None, 0o400))
    ...
    nan_raw = canonical_bytes(valid).replace(b'"like_authorized":true', b'"like_authorized":NaN')
...
def validate_public_schema() -> None:
    ...
    assert schema['properties']['schema']['const'] == 'taey_linkedin_private_comment_approval_v1'
...
def validate_static_boundary() -> None:
    ...
    assert "private_root / 'transactions' / seat_id / f'{correlation_id}.json'" in source
    assert "private_root / 'gate-receipts' / seat_id / f'{correlation_id}.json'" in source
    assert source.index("write_new_private_json(paths['receipt'], receipt)") < source.index(
        "write_new_private_json(paths['transaction'], transaction)"
    )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix='linkedin-comment-materialization-validator-') as raw:
        base = Path(raw)
