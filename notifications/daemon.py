...
def inject_via_tmux(session: str, message: str) -> bool:
    ...
    Uses the proven mcp-reconnect pattern:
    1. C-a C-k to clear stale input
    2. -l flag for literal mode (no key interpretation)
    3. Enter as separate send-keys call
    ...
    try:
        ...
        # Submit: Enter + C-j (Kitty protocol fallback)
        subprocess.run(
