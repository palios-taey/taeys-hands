...
4. ONLY escalate if `next --platform <name>` prints "Sweep found no new items":
   taey-notify weaver "ESCALATION from $(hostname): Weaviate sweep empty, truly done" --type escalation
```
...
- **Any infrastructure error**: DO NOT try to fix. Escalate to Spark immediately:
  ```
  taey-notify weaver "ESCALATION from $(hostname): <problem>" --type escalation --priority high
  ```
