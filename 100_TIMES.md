...
## 11. ChatGPT SEND — the button IS in the tree; find it by exact name + use the ACTION interface
- CORRECTION (Jesse, 2026-06-01): the composer/send/stop buttons ARE in the AT-SPI tree. Proven: a doc scan while generating returns `[push button] 'Stop answering'`. Do NOT conclude "outside the tree" and do NOT fall back to screen-coordinate clicks. That was my error.
- **Why my scans "found nothing":** (1) I FILTERED on readable extents — but these ChatGPT composer buttons return `getExtents()=None`, so any `y>NNN` / geometry filter excludes ALL of them. NEVER gate a button scan on extents for ChatGPT. (2) I grepped the name `'send'` — the send button's exact accessible name is different; capture it from a live scan when the composer has staged text (idle, not generating).
- **Correct actuation:** find the button by its EXACT accessible name (per the YAML element_map), then activate via the AT-SPI **ACTION interface** — `obj.queryAction().doAction(0)` (a11y "press"/"click" action) — NOT coordinate clicks (extents are None) and NOT guessed screen pixels. If do_action is confirmed not to activate for a given button, that's a per-element finding to record — but verify it, don't assume.
- Capture the staged send button's exact name+role into `chatgpt.yaml element_map.send_button` from a live scan (composer with text). Stop button is `'Stop answering'` (also verify the other stop names live).
- Single-attempt discipline still applies: try the action, screenshot-verify (URL chatgpt.com/ → /c/<id> + Stop button), and if it didn't fire, re-scan the tree (observation) — don't blind re-click or screen-coord flail.
