...
### Spark #2 (Worker - 192.168.100.11, external 10.0.0.80)
...
### Spark #1 (Head - 192.168.100.10, external 10.0.0.68)
...
**Access Method**:
```bash
# From Mac: SSH timeout to 10.0.0.68
# From Spark 2: ssh spark@192.168.100.10 (works)
```
...
**Code References**:
- `unified_query_canonical.py:75` → `http://192.168.100.11:8001/v1`
- `workshop_unified_loader_v2.py:64` → `http://192.168.100.11:8001/v1`
...
curl http://192.168.100.11:8001/v1/embeddings \
  -H "Content-Type: application/json" \
  -d '{"input": "test", "model": "Qwen/Qwen3-Embedding-8B"}'

# Test Weaviate
curl http://192.168.100.11:8080/v1/meta

# Test Neo4j connection
python3 << 'PYTHON'
from neo4j import GraphDatabase
driver = GraphDatabase.driver('bolt://192.168.100.10:7687')
...
├─ 10.0.0.68  Spark #1 (FIREWALLED from Mac, timeout)
└─ 10.0.0.80  Spark #2 (ACCESSIBLE from Mac)

Internal Network (192.168.100.x):
├─ 192.168.100.10  Spark #1 HEAD (Ray cluster, Neo4j, ES, Coder)
└─ 192.168.100.11  Spark #2 WORKER (Weaviate, Embedding)
