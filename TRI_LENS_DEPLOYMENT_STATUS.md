...
### Spark #2 (Worker - REDACTED_LAN_IP, external 10.x.x.80)
...
**Missing**:
- ❌ builder-taey repository not at `<OPERATOR_HOME>/builder-taey/`
...
### Spark #1 (Head - REDACTED_LAN_IP, external 10.x.x.68)
...
# From Spark 2: ssh user@REDACTED_LAN_IP (works)
```
...
**Code References**:
- `unified_query_canonical.py:75` → `http://REDACTED_LAN_IP:8001/v1`
- `workshop_unified_loader_v2.py:64` → `http://REDACTED_LAN_IP:8001/v1`
...
Type=simple
User=root
WorkingDirectory=<OPERATOR_HOME>
...
curl http://REDACTED_LAN_IP:8001/v1/embeddings \
  -H "Content-Type: application/json" \
  -d '{"input": "test", "model": "Qwen/Qwen3-Embedding-8B"}'

# Test Weaviate
curl http://REDACTED_LAN_IP:8080/v1/meta

# Test Neo4j connection
python3 << 'PYTHON'
from neo4j import GraphDatabase
driver = GraphDatabase.driver('bolt://REDACTED_LAN_IP:7687')
...
```
External Network (REDACTED_LAN_IP):
├─ 10.x.x.68  Spark #1 (FIREWALLED from Mac, timeout)
└─ 10.x.x.80  Spark #2 (ACCESSIBLE from Mac)

Internal Network (REDACTED_LAN_IP):
├─ REDACTED_LAN_IP  Spark #1 HEAD (Ray cluster, Neo4j, ES, Coder)
└─ REDACTED_LAN_IP  Spark #2 WORKER (Weaviate, Embedding)

200GbE, 0.162ms latency
```

**Service Communication**:
- All cross-node communication uses internal IPs (REDACTED_LAN_IP)
