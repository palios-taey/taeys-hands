...
class PhiCompressor:
    ...
    Architecture:
    . Embed all chunks using provided embedder
    . Build semantic similarity graph (cosine similarity)
    . Compute graph Laplacian and its eigenvectors (harmonics)
    . Score each chunk by its participation in top-k φ-weighted harmonics
    . Select highest-scoring chunks up to target ratio
    . Output concatenated text (preserving original order)
...
def create_ollama_embedder(
    base_url: str = "http://10.0.0.80:11435",
