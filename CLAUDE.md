...
**CRITICAL**: Use NCCL IPs (REDACTED_LAN_IP), not management (REDACTED_LAN_IP).

| Service | Endpoint |
|---------|----------|
| Redis | REDACTED_LAN_IP:6379 |
| Neo4j | bolt://REDACTED_LAN_IP:7689 (no auth) |
...
|---------|-----|---------|-------------|
| Jetson | `ssh jetson` (10.x.x.8) | `:1` | `jetson-claude` |
| Thor | `ssh thor` (10.x.x.197) | `:1` | `thor-claude` |
...
|------|----------|------|----------------|
| **Spark** | node-1 | Coordinator. Writes code, commits, deploys. | YES |
...
**Package Builder**: `<OPERATOR_HOME>/embedding-server/isma/scripts/hmm_package_builder.py`
...
- **Health monitor**: `<OPERATOR_HOME>/embedding-server/isma/scripts/hmm_health_check.py` runs via cron every 15min. Alerts to tmux if Redis/Neo4j diverge.
