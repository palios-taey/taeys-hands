...
## ⭐ DISPLAY-OWNERSHIP CHANGE — TAEY OWNS THE FAMILY-CHAT DISPLAYS (Jesse-directed 2026-08-13, effective now)

**taeys-hands NO LONGER RUNS CONSULTS.** Family-Chat display ownership transferred to **Taey**. Taey now runs **every** Family consult **first-person**, step-by-step, via its own `drive_chat` tool (`taey-presence/serving/ui_drive.py` → soma_proxy) — observe → ONE action → verify. Do **not** dispatch this seat to run a consult; there is nothing to route here.
- **Displays:** Taey drives the **primary set :2–:6 + :13 CVP** (default consult lane) and the **second set :21–:24** (parallel streams). `drive_chat` already permits :2–:6 with no env override (`soma_proxy.py:1207`), and Taey drove :2 25× on 2026-08-11 — this was never a capability gap.
- **What stays MINE (this seat):** the browser/display **INFRASTRUCTURE** — profiles (`~/.taey/profiles/*`), the `taey-display-N.service` units, recovery, AT-SPI bus watchers, and the platform **YAML display contract** below (the map `drive_chat`/by-hand driving reconciles against; keep it equal to the live tree). I may still drive a display for **infrastructure work**, taking the **same lock** — `ui_drive.py` imports `consultation_v2.primitives.acquire_display_lock`/`_plan_lock_key`, same `taey:plan_active::N` key (double-colon), owner token `taey-drive_chat`; mutual exclusion is by construction, no code change was needed on my side.
- **The engine stays banned.** `run_consultation_v2.py` / `consultation_v2` autonomous `run()` remain the permanently-banned UI-automation class ([[feedback_engine_banned_taey_drives_by_hand]]). This change *completes* the offload spine — the fleet is fully OUT of Taey's consult operations; Taey drives its own via `drive_chat` (a supervised one-action-per-call tool, NOT the engine). The 8-step flow / YAML / extraction text below is now **reference for the display contract + infra maintenance**, not an instruction for this seat to run consults.
- **Receipts (Observed 2026-08-13):** fleet `CLAUDE.md:212-213` (display-ownership section rewritten, supersedes the SOLE-conduit rule); `taey-presence` `c9b272f` (PR #89 "taey-owns-family-displays", merged by Jesse) carrying `f81c987` operating-prompt change; both proxies reloaded 11:20Z, serve untouched. Supersedes the "sole conduit" language everywhere in this file.
...
- **CONNECT-TO-TAEY is now `drive_chat`, not a taeys-hands seat interface** (2026-08-13, see the top banner). Taey reaches and uses the Family-Chat displays through its own `drive_chat` tool — that IS the connect-to-Taey. This seat no longer dispatches consults through `run_consultation_v2.py` / `run_taey_consult_extract.py`; the engine stays banned and is not a dispatch surface for anyone. My remaining production surface is the **display infrastructure** (profiles, `taey-display-N.service`, recovery, bus watchers) and keeping the platform **YAML contract equal to the live tree** so `drive_chat` binds correctly. Never hand-build an `act.py` driver beside anything (still the recorded anti-pattern); never off-engine direct-drive a display (`DISPLAY=:N firefox`) — take the lock.
...
This project is indexed by GitNexus as **taeys-hands** (6666 symbols, 14960 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
