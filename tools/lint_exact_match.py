...
matches by exact `name` + `role`. NO name_contains / names_any_of /
...
Usage:
  python3 tools/lint_exact_match.py [--staged] [paths...]
Default paths: platforms/*.yaml consultation_v2/platforms/*.yaml
"""
import sys
import glob
import subprocess
...
FORBIDDEN = ("name_contains", "names_any_of", "role_contains", "url_contains",
...
def _default_paths() -> list:
    paths = sorted(glob.glob("platforms/*.yaml"))
    paths += sorted(glob.glob("consultation_v2/platforms/*.yaml"))
    return paths
