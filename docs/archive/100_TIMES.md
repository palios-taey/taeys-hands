...
- **A GREEN systemd unit PROVES NOTHING about the tree.** `systemctl --user is-active taey-display-N` = ACTIVE while the AT-SPI document tree is EMPTY (0 nodes) is a real, recurring state — the unit is up, the firefox is alive, but the page/a11y bridge is dead. NEVER declare a display healthy off unit status; prove it with a HARD AT-SPI node count (0 vs 200+) + a screenshot showing the page rendered + `xprop AT_SPI_BUS` guid == `/tmp/a11y_bus_:N` contents. Diagnose → fix → verify mechanically. (2026-07-26: :6 read ACTIVE with 0 nodes.)
- **Empty-tree has TWO DISTINCT CLASSES — do not conflate them:** (a) **Dead-renderer on a long-lived firefox** — bus-file guid MATCHES xprop (not a stale-bus case), firefox has been up days, its renderer + a11y bridge died → blank page → 0 nodes. **Restart-fixable:** `systemctl --user restart taey-display-N` relaunches a fresh firefox that re-renders + re-registers (2026-07-26 :6: up 7 days since Jul 19, restart took it 0→207 nodes, logged-in render restored). (b) **Persistent a11y-registration failure (grok :5)** — the empty tree SURVIVES a full restart; a restart just gives you a green unit and STILL an empty tree, which is the exact trap class (a) sets. **NOT restart-fixable; it is a separate focused-debug item.** Before restarting to "fix" an empty tree, know which class you are in, and re-verify the node count AFTER — a restart that leaves 0 nodes means class (b), stop and debug, don't declare victory.
