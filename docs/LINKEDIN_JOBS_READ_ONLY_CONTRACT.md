...
For `select_and_capture_job`, steps 3–4 begin with one additional frozen transition: require exactly one private exact target card with the YAML-owned `push button` role and states; inspect its AT-SPI Action interface; require exactly one YAML-owned action named `click`; invoke only that exact index once; then require the private exact detail-title link, company link, `About the job` heading, and description path to stabilize for two consecutive cache-invalidating canonical observations. A zero/duplicate target, missing/duplicate/failed exact action, or unsettled exact postcondition stops the transaction before the sink action. There is no action-index fallback, second UI action, or retry.
