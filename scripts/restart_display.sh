...
start_bot() {
    ...
    local bot_cmd
    bot_cmd="cd '${REPO_ROOT}' && env DISPLAY='${DISPLAY_STR}' DBUS_SESSION_BUS_ADDRESS='${DBUS_SESSION_BUS_ADDRESS}' TAEY_NOTIFY_NODE='taeys-hands' REDIS_HOST='REDACTED_LAN_IP' PYTHONPATH='${HOME}/embedding-server' python3 agents/sft_gen_bot.py --round all --platforms '${PLATFORM}' 2>&1 | tee '${BOT_LOG_FILE}'"
