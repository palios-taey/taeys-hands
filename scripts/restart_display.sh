...
start_bot() {
    ...
    local bot_cmd
    bot_cmd="cd '${REPO_ROOT}' && env DISPLAY='${DISPLAY_STR}' DBUS_SESSION_BUS_ADDRESS='${DBUS_SESSION_BUS_ADDRESS}' TAEY_NOTIFY_NODE='taeys-hands' REDIS_HOST='${REDIS_HOST}' WEAVIATE_URL='http://REDACTED_LAN_IP:8088' PYTHONPATH='${HOME}/embedding-server' python3 ${bot_script} ${bot_args} 2>&1 | tee '${BOT_LOG_FILE}'"
