...
USERJS
    ...
    if [ "$SKIP_BOTS" = "false" ]; then
        ...
        tmux send-keys -t "$tmux_session" \
            "cd $TAEY_PATH && DISPLAY=:$display DBUS_SESSION_BUS_ADDRESS=$DBUS REDIS_HOST=REDACTED_LAN_IP PYTHONPATH=$EMBEDDING_PATH python3 agents/sft_gen_bot.py --round $ROUND --platforms $platform 2>&1 | tee /tmp/sft_${platform}.log" Enter
