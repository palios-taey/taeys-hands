...
def _artifacts(source: dict[str, Any], receipt_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
...
def _validate_artifacts(
    ...
    try:
        ...
        transaction_bytes = read_owned_private_bytes(paths['transaction'], 'transaction')
    ...
    if receipt_sha256 != expected_receipt_sha256 or transaction_sha256 != expected_transaction_sha256:
...
def _prepare(paths: dict[str, Path], source: dict[str, Any]) -> dict[str, str]:
    ...
    for name in ('receipt_root', 'receipt_parent', 'transaction_root', 'transaction_parent'):
...
def build_parser() -> argparse.ArgumentParser:
    ...
    for command in ('prepare', 'preflight'):
        ...
        if command == 'preflight':
            subparser.add_argument('--expected-gate-receipt-sha256', required=True, type=_sha256)
            subparser.add_argument('--expected-transaction-sha256', required=True, type=_sha256)
    return parser
