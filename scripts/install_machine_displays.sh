...
ExecStart=/usr/bin/dbus-run-session -- /usr/bin/bash -lc 'echo "\$DBUS_SESSION_BUS_ADDRESS" > /tmp/dbus_session_bus_:${display_num}; /usr/libexec/at-spi-bus-launcher --launch-immediately & for i in \$(seq 1 10); do A11Y_ADDR=\$(xprop -display :${display_num} -root AT_SPI_BUS 2>/dev/null | sed "s/.*= \\"//;s/\\"\$//"); [ -n "\$A11Y_ADDR" ] && echo "\$A11Y_ADDR" > /tmp/a11y_bus_:${display_num} && break; sleep 1; done; AT_SPI_BUS_ADDRESS="\${A11Y_ADDR}" /usr/libexec/at-spi2-registryd & sleep 1; openbox & sleep 1; ${vnc_block}mkdir -p \$HOME/.taey/profiles/${profile}; cp ${FIREFOX_USER_JS_SRC} \$HOME/.taey/profiles/${profile}/user.js; echo "user_pref(\\"accessibility.force_disabled\\", -1);" >> \$HOME/.taey/profiles/${profile}/user.js; GTK_USE_PORTAL=0 LIBGL_ALWAYS_SOFTWARE=1 GNOME_ACCESSIBILITY=1 GTK_MODULES=gail:atk-bridge AT_SPI_BUS_ADDRESS="\${A11Y_ADDR}" ${FIREFOX_BIN} --no-remote --profile \$HOME/.taey/profiles/${profile} "${url}" & FIREFOX_PID=\$!; echo "\${FIREFOX_PID}" > /tmp/firefox_pid_:${display_num}; wait \${FIREFOX_PID}'
