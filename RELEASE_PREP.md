...
**Status: PUBLIC — history-scrubbed and independently verified clean** (`palios-taey/taeys-hands`). This doc tracks the PRIVATE_TO_PUBLIC prep that took it public. History was rewritten twice (IPs/paths/profile-names, then a corrective pass for PII emails) and re-verified on a fresh public mirror via `git rev-list --all` ancestry: emails / LAN IPs / operator paths / profile names / `archive/` / `refs/pull/*` all 0; only `main` exists.
...
- **Live code/config: nearly clean already.** Only 4 hits, all benign:
  ...
  - **No LAN IPs (REDACTED_LAN_IP / REDACTED_LAN_IP), no hardcoded DB password, no secrets file, no API keys in live code.**
