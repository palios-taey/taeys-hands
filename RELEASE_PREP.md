...
This engine drives **logged-in, authenticated** AI accounts (ChatGPT Pro / Claude / Gemini / Grok / Perplexity) via Linux AT-SPI, framed in internal docs as "undetectable browser control." Publishing a tool whose purpose is automating authenticated consumer-AI accounts is a real ToS / account-safety / optics question. Minimum before publish: reframe as accessibility automation of *your own* accounts, and a deliberate Family decision on whether it should be public at all. **Open — Jesse + Family.**

## Discovery — full scan 2026-06-19 (observed)
- **Secrets: CLEAN.** gitleaks over full history (1286 commits) = 86 hits, **all verified false positives** (training-config keys in `agents/sft_tracker.py`; code identifiers like `getattachtriggerkey`/`ATTACHKEYS` inside AI audit-response markdown). No real credentials in current tree or history. `.gitleaks.toml` allowlist (narrow, documented) → scan now reports **0**. *(Sampled hits observed directly; rest inferred same class.)*
