...
def load_and_prepare_items(
    ...
    for idx, (filepath, source_name) in enumerate(file_list):
        ...
        # Read file
        if filepath.startswith(('mira@', 'user@')):
...
async def main():
    ...
    parser.add_argument("--spark1", action="store_true", help="Load from <OPERATOR_HOME>")
    ...
    parser.add_argument("--expansion-local", action="store_true", help="Load from synced expansion files at <OPERATOR_HOME>/data/expansion_md (includes backups)")
    ...
    if args.spark1 or args.all:
        file_list.extend(find_local_md_files("<OPERATOR_HOME>", "spark1"))
    ...
    if args.mira or args.all:
        file_list.extend(find_remote_md_files("10.x.x.163", "mira", "<OPERATOR_HOME>", "mira"))
    if args.mira_local:
        file_list.extend(find_local_md_files("/tmp/mira_md_files", "mira"))
    if args.expansion_local:
        file_list.extend(find_local_md_files("<OPERATOR_HOME>/data/expansion_md", "expansion", skip_backup_exclusions=True))
